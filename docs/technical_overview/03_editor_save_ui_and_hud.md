...
- Основной toolbar editor упрощён до двух кнопок (`Build`, `Gases`): фон панели полностью прозрачный, размер кнопок/иконок увеличен, а расстояние между кнопками увеличено.
- Иконки `Build/Gases` на main-toolbar увеличены до `32px` при размере кнопок `48px`, чтобы инструменты читались быстрее на рабочем экране.
...
- Подпанели `Build` и `Gases` переведены на общий `PanelManager`: отдельные панели с заголовком инструмента в `BottomLeft`, кнопкой `X` в header, лимитом высоты `<= 1/2` экрана и автоскроллом при переполнении.
- Контент `Build`/`Gases` внутри этих панелей рендерится таблицей в 3 колонки (`left -> right`, `top -> bottom`), а иконки вариантов заметно увеличены.
- Варианты клеток в панели `Build` теперь используют обычные cell-спрайты (`tile_brick`/`tile_metal`) вместо силуэтных превью.
