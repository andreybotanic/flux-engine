...
- Основной toolbar editor теперь строится динамически из зарегистрированных категорий сущностей: базовые `Cells` и `Gases`, плюс категории из enabled content-плагинов.
- Toolbar остаётся прозрачным (видны только кнопки), кнопки имеют размер `48px`, иконки `32px`, межкнопочный gap увеличен до `16px`.
- Кнопка `Build` переименована в категорию `Cells`; `Gases` сохранена как отдельная категория.
- Инструмент `Erase` убран с основной панели и переключается по `X`; инструмент `Scissors` переведён на `C` с тем же двусторонним toggle-поведением.
- Панели вариантов теперь создаются по категориям через `PanelManager`: `BottomLeft`, заголовок = label категории, custom close (`X`), лимит высоты `<= 1/2` экрана и автоскролл при переполнении.
- Контент панели категории рендерится таблицей в 3 колонки (`left -> right`, `top -> bottom`) с увеличенными кнопками вариантов (`72px`) и иконками (`48px`).
- Категории/сущности без назначенной категории не показываются в category-panel UI.
- Варианты клеток в категории `Cells` используют обычные cell-спрайты (`tile_brick`/`tile_metal`) вместо силуэтных превью.
- Ghost-preview для `Cells` выбирает silhouette/image напрямую из `ContentRegistry` по выбранному `CellMaterial`, без hardcoded ветвления `brick/metal` в editor-коде.
...
- `PlacedStructure` хранит `PackedState(u16)`: для `Pipe` это bitmask `0..15`, для `Bridge` — состояние ориентации (`0` горизонталь, `1` вертикаль).
- World-render для `Pipe/Vent/Bridge` идёт через общий путь `structure -> state -> sprite(+transform)` без отдельного дублирующего legacy world-слоя.
...
- `load_save(...)` принимает только текущую schema `7`; старые версии сейвов больше не конвертируются и считаются несовместимыми.
...
- В базовом блоке `Cell` больше не выводится строка `State`; состояние твёрдой сущности показывается только через отдельный entity-блок материала.
- В `plugin SDK -> host runtime` добавлена поддержка title-only HUD-блоков: пустой `lines` больше не отбрасывается при ABI-submit, поэтому material-блоки (`Brick/Metal`) доходят до UI без фиктивных строк.
...
- Если несколько material-дескрипторов используют общий visual TOML, label в `CellVisualPlacementConfigMap` переопределяется из descriptor-метаданных, чтобы HUD показывал имя конкретной сущности, а не общий label файла.
