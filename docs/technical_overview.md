...
- Для ghost-предпросмотра `BuildSolid` (Brick/Metal) используются отдельные silhouette-ассеты из `assets/sprites/world/`, а не world-тайлы материалов.
...
### Pipe layout и вентиляции

- Добавлен отдельный ресурс `PipeGrid`, который хранит pipe-layout независимо от legacy `GasStructureGrid`.
- Каждая клетка `PipeGrid` хранит:
  - наличие трубы;
  - битовую маску соединений по 4 направлениям;
  - наличие вентиляции.
- Разрешено сочетание `pipe + vent` в одной клетке.
- Труба может сосуществовать с обычной непроницаемой `Solid`-клеткой, но не с граничной `Boundary`.
- Вентиляция по-прежнему запрещена в `Solid`, `Boundary`, `Gas Source` и `Gas Sink`.
- Соединения между соседними pipe-клетками всегда симметричны: изменение одной стороны обязано зеркально обновлять вторую.
- Удаление pipe-клетки очищает все соседние связи до сброса самой клетки.
- Вентиляция без трубы допустима как layout-состояние, но считается нефункциональной и не участвует в pipe-симуляции.

### Газ внутри труб

- Добавлен отдельный ресурс `PipeGasField`, полностью изолированный от обычного `GasField`.
- Газ в world-клетке и газ в pipe-клетке одной и той же позиции не смешиваются напрямую.
- Обмен возможен только через вентиляцию в составе pipe-тика.
- Для каждой pipe-клетки действует лимит `1000` частиц суммарно по всем газам.
- Состояние pipe-газа сериализуется отдельно от обычного газа.
...
- Один pipe-тик выполняется в фиксированном порядке:
  1. Собрать connected components pipe-сети.
  2. Найти functional vents (`has_pipe && has_vent`) в каждой компоненте.
  3. Снять snapshot pipe-газа начала тика (`starting_pipe_totals` и `starting_pipe_species`) по всем сегментам компоненты.
  4. Для functional vent-клеток дополнительно снять snapshot world-газа той же клетки в rounded particles.
  5. По состоянию начала тика сформировать локальные заявки `pipe -> pipe`: сегмент может толкать газ только в соединённого соседа с меньшим локальным заполнением.
  6. По состоянию начала тика сформировать локальные заявки `pipe -> world` и `world -> pipe` только для вент-клеток; здесь используется порог `MIN_VENT_PRESSURE_DELTA_PARTICLES = 5`.
  7. Для сегмента с несколькими downstream-направлениями сначала посчитать один общий исходящий budget по усреднённому локальному перепаду, а уже потом детерминированно разделить этот budget между направлениями по их дефицитам.
  8. Ограничить входящие заявки свободной ёмкостью целевого pipe-сегмента, считая её синхронно от snapshot-состояния начала тика и уже принятого исходящего потока из этой клетки.
  9. Собрать next-buffer pipe-газа: вычесть принятые исходящие смеси из источников и добавить принятые входящие смеси в приёмники.
  10. Выполнить только один слой внутритрубных переносов `source -> сосед` за тик; газ, пришедший в клетку в этом тике, не может уйти дальше до следующего тика.
  11. Применить world↔vent обмен по принятым заявкам той же синхронной фазы.
  12. Записать фактические соседние переносы в `PipeFlowVisualState` для анимации `F3`.
- Pipe-подсистема больше не использует глобальный solver по вентиляциям: “давление” трубы локально равно суммарному числу частиц в конкретном pipe-сегменте.
- Локальная модель официально допускает работу трубы даже с одной вентиляцией: если pipe-сегмент у вентиляции пустой, а world-клетка богаче газом, вентиляция начинает постепенно втягивать газ в сеть.
- Порог `5` частиц применяется только к обмену `world ↔ vent`; для переноса `pipe ↔ pipe` отдельного порога нет.
- Для локального переноса между соседними pipe-сегментами используется фиксированная доля локального перепада `1/2`, после чего поток дополнительно ограничивается доступной массой и вместимостью.
- Разветвления, петли и параллельные маршруты не требуют special-case solver-а: поток делится детерминированно по локальным дефицитам downstream-сегментов, но общий исходящий объём сегмента считается один раз на всю развилку, чтобы перекрёсток не опустошался целиком за один тик.
- Клетка `pipe + vent` считается обычным сегментом трубы: в первый тик поток заполняет её pipe-объём, а уже на следующих тиках этот газ начинает продвигаться дальше по сети.
...
- Пропорциональное всасывание/выпуск и ограничение ёмкости используют детерминированное целочисленное распределение со стабильным tie-break.
...
- В editor добавлена кнопка `Gases` на основной панели инструментов рядом с `Build`.
- При выборе `Gases` открывается отдельная подпaнель с режимами `Pipe` и `Vent`, по аналогии с `Build -> Brick/Metal`.
- Режим `Scissors` переключается клавишей `X` и удаляет только связь между двумя соседними pipe-клетками, не удаляя сами клетки.
- Drag-построение и drag-разрезание используют grid-traversal между предыдущей и текущей клеткой курсора, чтобы исключить пропуски на быстром движении мыши.
- Существующий `EraseSolid` очищает pipe-клетку, вентиляцию и pipe-gas в выбранной клетке.
...
- Save-формат дополнительно расширен chunk-ами:
  - `pipe_layout` (`pipe_layout.bin`, `binary_v1`);
  - `pipe_gas` (`pipe_gas.bin`, `binary_v1`).
- Версия схемы сохранения повышена до `3`.
...
- В runtime snapshot теперь входят:
  ...
  - `PipeLayoutSnapshot`;
  - `PipeGasSnapshot`.
- Загрузка старых save-файлов схемы `2` остаётся допустимой: при отсутствии pipe-chunk pipe-подсистема восстанавливается пустой.
...
- В обычном мире pipe-спрайты рисуются ниже непроницаемых world-клеток, поэтому труба может визуально лежать “внутри” стены; в `F3` pipe-layer поднимается выше solid-спрайтов.
- Tint труб и vent overlay в `F3` намеренно сделан почти белым, чтобы pipe-сеть не терялась на затемнённом фоне.
- Статический газ в трубе рисуется процедурно как два квадрата: белая рамка и цветной внутренний fill. Размер рамки масштабируется по фактическому количеству газа в диапазоне `22% .. 70%` клетки, а при нулевом количестве газовый квадрат не рисуется.
...
- HUD инспектора клетки динамически расширяет свою высоту под содержимое и, если под курсором есть труба, добавляет отдельный pipe-блок с газами внутри трубы и строкой `Pipe total: N / 1000`.
