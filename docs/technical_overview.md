...
- Для enabled runtime DLL-плагинов дополнительно создаётся `RuntimeDllPluginRegistry`: он держит DLL library + plugin handle живыми, кеширует typed event handlers по `PluginEventKind` и вызывает только те named exports, которые плагин явно зарегистрировал через ABI v4.
...
- Generated Plugin SDK navigation is grouped by API role: `Structures`, `Enums`, `Constants`, `Methods` and `Events`. Each concrete item gets its own generated page so the mdBook menu points to `FluxRuntimeHost`, `PluginEventKind`, `FLUX_PLUGIN_CREATE_EXPORT_NAME`, `WorldApi::get_cell_info` or `PluginEventKind::MouseDownCell` instead of internal Rustdoc sections such as `Fields`; `FluxOn*Fn` helper callback aliases stay in code for DLL loading but are intentionally hidden from the public Plugin SDK reference.
...
- Канонический plugin API живет в `src/plugins/api/` и считается Rust-first контрактом ядра. C ABI v4 разбит на `src/plugins/abi.rs` и `src/plugins/abi_events.rs`: первый файл содержит базовые host/registrar/runtime типы и export names, второй — typed event payload structs, event handler typedef-ы и mapping `PluginEventKind <-> ABI`.
