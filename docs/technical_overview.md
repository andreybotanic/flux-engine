...
- Номинальная ёмкость pipe-сегмента хранится в `PipeGasField` и берётся из `config/simulation.toml -> [pipe].segment_capacity_particles`; по умолчанию это `1000`.
...
- Для pipe solver-а введён отдельный конфиг `PipeSimulationConfig`:
  - `cell_volume_ratio`,
  - `segment_capacity_particles`,
  - `edge_transfer_per_tick`,
  - `vent_transfer_per_tick`,
  - `pressure_epsilon`.
- Давление в трубе считается как `pipe_particles / pipe_volume`, где `pipe_volume = cell_volume / cell_volume_ratio`. При дефолтном `cell_volume_ratio = 25` это означает, что `1000` частиц в трубе соответствуют давлению `25000` частиц в обычной world-клетке.
...
- Один pipe-тик выполняется в фиксированном порядке:
  ...
  5. По состоянию начала тика вычислить world/pipe pressure на вентиляциях и сформировать world↔vent budgets. `world -> pipe` остаётся локальным и не может быть ретранслирован дальше в тот же тик.
  6. Построить component-level transport plan:
     - source-driven заполнение тянет спрос от свободного объёма назад к inlet-вентиляциям;
     - sink-driven дренаж распространяет спрос от outlet-вентиляций назад по графу;
     - в чистом source-fill режиме промежуточный сегмент не начинает ретранслировать фронт дальше, пока сам не вышел на target fill, поэтому труба заполняется плотным фронтом, а не тонкой “плёнкой”.
  7. Если активного world↔vent обмена нет, использовать детерминированный local-equalization fallback: сегмент толкает газ только в соседа с меньшим заполнением, а общий исходящий budget сегмента считается один раз на всю развилку.
...
- Pipe-подсистема теперь использует гибридную модель:
  - pressure-driven component planner для компонент с активными вентиляциями;
  - local fallback для pipe-компонент без активного world↔vent обмена.
- Это позволяет:
  ...
  - тянуть газ к одиночной vacuum-вентиляции не только из первой клетки, а из всей connected component;
