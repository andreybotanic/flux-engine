...
- Для enabled runtime DLL-плагинов дополнительно создаётся `RuntimeDllPluginRegistry`: он держит DLL library + plugin handle живыми и вызывает optional `flux_plugin_on_event` по подписанным событиям.
...
- Plugin SDK documentation lives in `docs/plugin_sdk/` as an mdBook site. API reference chapters under `docs/plugin_sdk/src/generated/` are generated from SDK-facing Rustdoc sections and Rust AST metadata in `src/plugins/abi.rs`, `src/plugins/api/*`, `src/plugins/manifest.rs`, `src/plugins/id.rs`, `src/plugins/content.rs` and `src/plugins/substances.rs`.
...
- Live DLL executor хранит включённые runtime-плагины до reload/shutdown, вызывает `flux_plugin_on_event` и передаёт `FluxRuntimeHost` callbacks для `set_cell_material`, `add_gas`, готового overlay frame, HUD blocks и plugin save chunks.
...
- В `src/plugins/flux_api_*_demo_plugin/` добавлены небольшие runtime DLL fixtures для проверки v3 execution path: cell demo рисует линию metal-клеток из right-button mouse events, tick demo добавляет направленный H2 в фиксированной клетке, temperature overlay отдаёт готовый heatmap frame, UI/save demo добавляет HUD-блок, увеличивает per-cell счётчик по left-click и пишет save chunk.
