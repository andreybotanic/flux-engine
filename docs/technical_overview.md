...
- Один pipe-тик выполняется в фиксированном порядке:
  ...
  4. Посчитать давление вентиляций по обычному `GasField` внешнего мира.
  5. Решить потенциалы внутри pipe-графа итерационным solver-ом с одинаковым сопротивлением каждого ребра.
  6. Вычислить желаемые межклеточные потоки от большего потенциала к меньшему.
  7. Ограничить их доступным газом, вместимостью pipe-клеток и детерминированным округлением.
  8. Забрать газ из world-клеток во входные вентиляции пропорционально составу смеси.
  9. Переместить газ по pipe-рёбрам.
  10. Выпустить газ наружу через выходные вентиляции пропорционально составу pipe-смеси.
  11. Сохранить transfer-данные для визуальной анимации в `F3`.
- Если давления у подключённых вентиляций выравнены, pipe-граф не создаёт направленного потока.
...
- Overlay-символ вентиляции для `F3` генерируется отдельно как контурный квадрат с двунаправленной вертикальной стрелкой и жёлтым tint; обычный world-спрайт вентиляции при этом тоже остаётся видимым под иконкой.
