...
- Для enabled runtime DLL-плагинов дополнительно создаётся `RuntimeDllPluginRegistry`: он держит DLL library + plugin handle живыми, кеширует typed event handlers по `PluginEvent` и вызывает только те named exports, которые плагин явно зарегистрировал через ABI v4.
...
- Plugin SDK documentation lives in `docs/plugin_sdk/` as an mdBook site. API reference chapters under `docs/plugin_sdk/src/generated/` are generated from SDK-facing Rustdoc sections plus Rust AST metadata in `src/plugins/abi.rs`, `src/plugins/abi_events.rs`, `src/plugins/api/*`, `src/plugins/manifest.rs`, `src/plugins/id.rs`, `src/plugins/content.rs`, `src/plugins/substances.rs` and allowlisted SDK-facing types from `src/world/grid.rs`, `src/world/structures.rs`, `src/config/mod.rs`, `src/config/hud.rs` and `src/render/world_view.rs`.
- Generated Plugin SDK navigation is grouped by API role: `Structures`, `Enums`, `Constants`, `Methods` and `Events`. Each concrete item gets its own generated page so the mdBook menu points to `FluxRuntimeHost`, `FluxRuntimeHost::set_cell_material`, `FLUX_PLUGIN_CREATE_EXPORT_NAME` or `PluginEvent::MouseDownCell` instead of internal Rustdoc sections such as `Fields`; legacy `FluxOn*Fn` aliases stay in code for DLL loading, while raw host callback typedefs are hidden behind wrapper methods on `FluxHostApi`, `FluxRegistrar` and `FluxRuntimeHost`.
...
- SDK examples are stored outside Rustdoc in `docs/plugin_sdk/src/examples/{methods,events,constants}/`. Generated pages include those snippets verbatim and link back to the source snippet file, which keeps Rust source comments short while still giving every method, callback, constant and event a concrete usage example.
- The SDK docs generator is intentionally strict: every documented SDK item must have a doc-comment summary plus its required sections (`# Fields`, `# Variants` or `# SDK Notes`), every public SDK-facing field and enum variant must have an explicit description, and every method/callback/event/export symbol must have a matching external example snippet. Missing descriptions or snippets make `cargo xtask generate-plugin-sdk-docs` and `cargo xtask check-plugin-sdk-docs` fail.
- Plugin SDK docs commands:
  - `cargo xtask generate-plugin-sdk-docs` rewrites generated Markdown chapters.
  - `cargo xtask check-plugin-sdk-docs` validates strict Rustdoc sections and fails when tracked generated docs are stale.
...
- Канонический публичный plugin API для runtime DLL живет в `src/plugins/abi.rs` и `src/plugins/abi_events.rs`. `abi.rs` содержит базовые host/registrar/runtime типы, их wrapper-методы и export names, а `abi_events.rs` — typed event payload structs, `FluxEventKind` и mapping `PluginEvent <-> ABI`.
- Plugin-facing данные и дескрипторы делятся на два слоя: ABI-совместимые сущности с префиксом `Flux*` (`FluxHostApi`, `FluxRegistrar`, `FluxRuntimeHost`, event payloads) и engine-side descriptor/model types без префикса `Plugin*` (`OverlayFrame`, `OverlayRenderPolicy`, `ToolDescriptor`, `PanelDescriptor`, `HudBlock`, `SaveChunkStore`), которые использует сам хост при обработке регистраций и рендера.
- Публичный Plugin SDK больше не показывает отдельные `WorldApi`/`WorldApiMut`: для runtime DLL каноническая поверхность проходит через typed event payload structs, `FluxEventKind` и wrapper-методы `FluxRuntimeHost`, `FluxHostApi` и `FluxRegistrar`.
- Runtime world mutations для DLL-плагинов проходят через методы `FluxRuntimeHost`: `set_cell_material`, `add_gas`, `write_save_chunk`, `read_save_chunk`, `submit_hud_block` и `submit_overlay_frame`. Внутри ядра они вызывают ABI callbacks и синхронизируют `WorldGrid`, `GasField`, save chunks, HUD blocks, overlay frame и GPU dirty-state.
...
- Runtime event модель по-прежнему использует `PluginRuntimeRegistry` как engine-side grouping по `PluginEvent`, но этот grouping строится из explicit handler registrations, а не из catch-all callback-а. В API есть события lifecycle/save/simulation/input/UI/render, но нет общих `CellChanged`, `CellGasChanged` и `CellMaterialChanged`.
...
- ABI v4 registrar принимает callbacks для explicit event handlers, tool descriptors, overlay descriptors, save chunk descriptors и gas substances. Для plugin-facing кода в `src/plugins/abi_events.rs` добавлен `#[repr(u32)] enum FluxEventKind`, поэтому плагин во время `flux_plugin_register` передаёт `FluxEventKind + handler_name`, а raw `u32` остаётся внутренней ABI-деталью `FluxEventHandlerDescriptor`. Loader валидирует identifier-style имя, отклоняет дубли `event_kind`/`handler_name` и проверяет наличие named export-а с ожидаемой typed signature.
- Live DLL executor хранит включённые runtime-плагины до reload/shutdown, резолвит typed handler pointers один раз при instantiation и затем dispatch-ит события напрямую в конкретный named handler без `flux_plugin_on_event`, передавая в обработчик `FluxRuntimeHost` как capability-object с методами для world mutation, overlay/HUD submission и plugin save chunks.
- World mutation callbacks собирают изменённые клетки в `RuntimeHostContext.changed_cells`; после dispatch системы отправляют `WorldCellChanged`, чтобы core render state синхронизировался так же, как после штатных editor-tools.
- `WorldBeforeSave` вызывается синхронно перед записью save-slot, чтобы plugin-owned chunks попали в `meta.toml` и отдельные файлы `plugin_chunks/*.bin`; при load chunks восстанавливаются в `SaveChunkStore`, а plugin может прочитать свой chunk через `read_save_chunk`.
- Plugin overlay hotkeys выбираются через `PluginRuntimeRegistry`: default overlays занимают `F1/F2/F3`, а runtime plugin overlays без явного hotkey получают следующий свободный slot в порядке регистрации включённых плагинов. Render-код не содержит знания о конкретных demo overlay IDs или `F4`.
- Plugin-controlled overlay render принимает от runtime DLL готовый RGBA8 frame через `submit_overlay_frame`; ядро только обновляет texture-backed sprite и не собирает demo overlay из per-cell `Gizmos`.
- В `src/plugins/flux_api_*_demo_plugin/` добавлены небольшие runtime DLL fixtures для проверки v4 execution path: cell demo обрабатывает right-button mouse events через отдельные named handlers, tick demo экспортирует named pre-step handler, temperature overlay использует `onRenderOverlay`, а UI/save demo разносит load/save/input/UI callbacks по отдельным ABI exports.
