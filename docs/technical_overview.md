...
- Один pipe-тик выполняется в фиксированном порядке:
  ...
  3. Отбросить компоненты, где вентиляций меньше двух.
  4. Посчитать давление вентиляций по обычному `GasField` внешнего мира в rounded particles.
  5. Построить список активных пар вентиляций: пара остаётся рабочей только если разность давлений не меньше `MIN_VENT_PRESSURE_DELTA_PARTICLES = 5`.
  6. Сформировать набор активных вентиляций как объединение всех активных пар; вентиляции вне этого набора в текущем тике не обмениваются газом с миром и не фиксируют boundary pressure в solver-е.
  7. Решить потенциалы внутри pipe-графа итерационным solver-ом с одинаковым сопротивлением каждого ребра, используя только активные вентиляции как граничные узлы.
  8. Вычислить желаемые межклеточные потоки от большего потенциала к меньшему.
  9. Ограничить их доступным газом начала тика, вместимостью pipe-клеток и детерминированным округлением.
  ...
  11. Выпустить газ наружу через активные выходные вентиляции пропорционально составу pipe-смеси, уже оказавшейся в vent-клетке после single-hop переноса.
  12. После этого подпитать входные активные вентиляции газом из world-клеток, пополняя их pipe-объём для следующего тика.
  13. Сохранить transfer-данные для визуальной анимации в `F3`.
- Если разность давлений у пары вентиляций меньше `5` частиц, эта пара не участвует в построении направленного потока.
- Разветвления, петли и параллельные маршруты обрабатываются одной conductance/resistance-моделью без special-case логики.
- Для снижения чрезмерной зависимости пропускной способности от длины пути на каждом pipe-ребре применяется фиксированный коэффициент проводимости `PIPE_FLOW_CONDUCTIVITY = 1.9`: он усиливает поток после расчёта локального перепада потенциалов, не меняя топологическое деление потока по сети.
...
- Для HUD и `F3` используется display-helper pipe-клетки: он показывает не только persistent `PipeGasField`, но и transient flow-пакеты последнего тика, чтобы активный поток не выглядел как `0 / 1000` в момент движения.
