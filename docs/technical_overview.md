...
- Plugin SDK documentation lives in `docs/plugin_sdk/` as an mdBook site. API reference chapters under `docs/plugin_sdk/src/generated/` are generated from SDK-facing Rustdoc sections and Rust AST metadata in `src/plugins/abi.rs`, `src/plugins/abi_events.rs`, `src/plugins/api/*`, `src/plugins/manifest.rs`, `src/plugins/id.rs`, `src/plugins/content.rs` and `src/plugins/substances.rs`.
- Generated Plugin SDK navigation is grouped by API role: `Structures`, `Enums`, `Constants`, `Methods` and `Events`. Each concrete item gets its own generated page so the mdBook menu points to `FluxRuntimeHost`, `PluginEventKind`, `FLUX_PLUGIN_CREATE_EXPORT_NAME`, `WorldApi::get_cell_info` or `PluginEventKind::MouseDownCell` instead of internal Rustdoc sections such as `Fields` or `SDK Example`.
...
- Structure pages render field tables from public struct fields, enum pages render variant tables, constant pages render declarations, method pages render arguments and return values from signatures or callback type aliases, and event pages render trigger descriptions plus payload arguments inferred from the matching `PluginEvent` variant.
- The SDK docs generator is intentionally strict: every documented SDK item must have a doc-comment with at least one section, and methods/callbacks/events/export symbols must include `# SDK Example`. Missing sections make `cargo xtask generate-plugin-sdk-docs` and `cargo xtask check-plugin-sdk-docs` fail.
