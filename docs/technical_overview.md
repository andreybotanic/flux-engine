...
- Один pipe-тик выполняется в фиксированном порядке:
  ...
  5. По состоянию начала тика сформировать world↔vent заявки. `world -> pipe` остаётся локальным и не может быть ретранслирован дальше в тот же тик.
  6. Если в компоненте есть sink-anchor (низкое давление мира на одной из вентиляций), собрать component-level demand-driven transport plan:
     - anchor задаёт downstream-demand;
     - спрос распространяется назад по кратчайшим pipe-рёбрам;
     - уже заполненные промежуточные сегменты могут запрашивать подпитку сверху, чтобы relay не затухал по длине трубы;
     - на развилках и параллельных путях upstream-бюджет делится детерминированно по downstream-дефициту.
  7. Если активного sink-anchor нет, использовать детерминированный local-equalization fallback: сегмент толкает газ только в соседа с меньшим заполнением, а общий исходящий budget сегмента считается один раз на всю развилку.
  8. Сначала синхронно снять весь pipe→pipe исходящий газ из source-узлов, затем синхронно распределить фактически принятые входящие смеси по target-узлам; отклонённый из-за вместимости остаток возвращается в source, а не теряется.
  9. Выполнить только один слой внутритрубных переносов `source -> сосед` за тик; газ, пришедший в клетку в этом тике, не может уйти дальше до следующего тика.
  10. Применить `pipe -> world`, затем `world -> pipe`.
  11. Записать фактические соседние переносы в `PipeFlowVisualState` для анимации `F3`.
- Pipe-подсистема теперь использует гибридную модель:
  - demand-driven component planner для компонентов, где есть активный sink-demand через вентиляцию;
  - local fallback для обычных pipe-компонент без такого downstream-спроса.
- Для компоненты с несколькими вентиляциями planner сначала вычисляет `head` как перепад между максимальным и минимальным `world`-давлением на её вентиляциях.
- Из этого `head` выводятся два параметра:
  - `target_fill` — до какого заполнения pipe-сегменты вообще должны стремиться при текущем перепаде;
  - `throughput_budget` — сколько газа может пройти через вход/выход вентиляции за тик.
- Для многовентиляционной компоненты сам `head` считается источником sustained-flow контракта: пока общий перепад между вентиляциями остаётся положительным, inlet/outlet не должны “пульсировать” только из-за того, что конкретная крайняя world-клетка на мгновение локально сравнялась с pipe-сегментом.
- Для компоненты с одним активным inlet-вентом planner строит направление потока от inlet к дальним клеткам и распространяет назад запрос на свободное место. Это позволяет в одном тике сделать цепочку вида `1 -> 2`, `2 -> 3`, а затем добрать новый газ из мира в `1`, если после исходящего потока там освободилась ёмкость.
- Для компоненты без активного inlet, но с outlet-вентом, отдельный sink-oriented planner тянет уже имеющийся газ назад к выпускной вентиляции через заполненные промежуточные сегменты.
- Это убирает сильное затухание давления в длинной трубе, но при этом не даёт компоненте оставаться искусственно “полной на 1000” после того, как общий перепад между входом и выходом уже почти исчез.
- Порог `5` частиц применяется только к world↔vent обмену.
- Local fallback сохраняет стабильное поведение на коротких изолированных трубах и не даёт им уходить в бесконечное перекидывание газа туда-сюда.
- Разветвления и параллельные пути остаются детерминированными: одинаковые маршруты делят поток честно, а более пустое направление получает больший share.
