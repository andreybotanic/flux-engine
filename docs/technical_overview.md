...
- Для внешнего затемнения используется отдельный спрайт-маска `assets/sprites/world/world_fade_mask.png` с непрерывным плавным градиентом alpha (без тайловой дискретизации), а сам слой маски размещён поверх world-тайлов.
