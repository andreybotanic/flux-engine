<!-- generated by cargo xtask generate-plugin-sdk-docs; do not edit by hand -->

<!-- FluxOnSimulationPreCellGasStepFn -->

# `FluxOnSimulationPreCellGasStepFn`

<span class="sdk-badge sdk-badge-ABI">ABI</span> <span class="sdk-kind">callback</span>

Source: **ABI v4 Events** (`src/plugins/abi_events.rs`). Generated group: **Methods**.

## Description

Function pointer type for `onSimulationPreCellGasStep`.

## Signature

```rust
pub type FluxOnSimulationPreCellGasStepFn = unsafe extern "C" fn (plugin : * mut FluxPluginHandle , event : * const FluxEmptyEventPayload , host : * mut FluxRuntimeHost ,) -> FluxStatus;
```

## Arguments

| Argument | Type | Description |
| --- | --- | --- |
| `plugin` | `* mut FluxPluginHandle` | `plugin` argument passed as `* mut FluxPluginHandle`. |
| `event` | `* const FluxEmptyEventPayload` | `event` argument passed as `* const FluxEmptyEventPayload`. |
| `host` | `* mut FluxRuntimeHost` | `host` argument passed as `* mut FluxRuntimeHost`. |

## Return Value

FluxStatus

## SDK Example

```rust
// Store or call the callback through the `FluxOnSimulationPreCellGasStepFn` ABI signature supplied by FluxEngine.
```

